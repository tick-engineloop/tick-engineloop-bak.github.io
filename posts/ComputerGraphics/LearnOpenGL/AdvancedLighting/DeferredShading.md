...
借助 G-Buffer 中的大量片段数据，我们就可以完全计算出场景的最终光照颜色。为此，我们在每个 G-Buffer 纹理上逐像素迭代，并将其内容用作光照算法的输入。由于 G-buffer 纹理值都是最终显示在屏幕上的场景片段的属性值，因此我们只需对每个像素执行一次昂贵的光照运算操作。这在复杂场景中特别有用，尤其是一些容易为每个像素调用多个昂贵片段着色器的前向渲染场景。
...
对于光照通道，我们将渲染一个填充屏幕的 2D 四边形（有点像后期处理效果），并在每个像素上执行昂贵的光照片段着色器：
...
在渲染之前，我们绑定了 G-buffer 的所有相关纹理，并将与光照相关的 uniform 变量发送到着色器。
...
光照通道的片段着色器与目前在光照章节中我们使用的着色器大致相似。不同的是获取光照输入变量的方法，现在我们直接从 G-buffer 中采样：
...
由于现在拥有计算 Blinn-Phong 光照所需的每片段变量（以及相关的 uniform 变量），因此我们不必对光照代码进行任何更改。在这里，延迟着色中唯一改变的是获取光照输入变量的方法。

Running a simple demo with a total of 32 small lights looks a bit like this:

运行包含 32 个小光源的一个简单示例，看起来有点像这样：
...
延迟着色的缺点之一是无法进行混合，因为 G-buffer 中每个片段位置上只保存了场景中最小深度的物体片段之属性，在同一位置上只有单个片段信息，而混合是对同一位置上不同深度的多个片段的组合操作。另一个缺点是延迟着色会迫使您对大多数场景的光照使用相同的光照算法;您可以通过在 G-buffer 中包含更多特定材质的数据来以某种方式缓解这种情况。
...
假设我们要将每个光源渲染为一个三维立方体，立方体位于光源的位置，发出光源颜色的光线。我们想到的第一个办法是，在延迟着色管线的末端，基于延迟光照产生的填充屏幕四边形之上，前向渲染所有光源。因此，这基本上就像我们通常做的那样渲染立方体，只是在完成延迟渲染操作后才进行。在代码中，这看起来是这样：
...
但是，这些立方体在被渲染的时候没有考虑延迟渲染器存储的任何几何深度，因此始终渲染在先前渲染的对象之上，这不是我们想要的结果。

<p align="center">
  <img src="../../../../images/LearnOpenGL-AdvancedLighting-DeferredShading-DeferredLightsNoDepth.png">
</p>
...
在这里，我们将整个读帧缓冲的深度缓冲内容复制到默认帧缓冲的深度缓冲;这同样可以用于颜色缓冲和模板缓冲。如果我们随后渲染光源立方体，立方体确实在场景的几何体上正确渲染：
...
通过这种方法，我们可以轻松地将延迟着色与前向着色相结合，以便应用混合效果和渲染需要特殊着色效果的对象，这很棒，但在纯粹的延迟渲染上下文中是不可能实现的。
...
延迟渲染最值得称赞的是，它能够在不造成严重性能损失的情况下渲染大量光源。延迟渲染本身不允许使用非常大量的光源，因为它仍然需要计算每个场景光源的每个片段的光照分量。为此我们应用了一个非常巧妙的优化：光体积，从而使大量光源在延迟渲染管线中出现成为可能。
...
光体积背后的想法是计算光源的影响半径或体积，即就是光能够到达片段的范围。由于大多数光源都使用某种形式的衰减，我们可以用这种衰减函数来计算光源的光能够达到的最大距离或半径。然后，只有当片段位于一个或多个这些光体积内时，我们才会进行昂贵的光照计算。这可以为我们节省一个可观的计算量，因为我们现在只计算必要的光照。
...
我们要做的是当 $F_{light}$ 等于 0.0 时求解这个方程中的 $d$ 。然而，这个方程永远不会真正等于 0.0，因此不会有解。所以，我们不会求表达式亮度值等于 0.0 时候的解，而是求靠近 0.0 时的解，这时候亮度仍然可以被看做是黑暗的。对于本章的示例场景，亮度值 5/256 是可以接受的，除以 256 是因为默认的 8 位帧颜色缓冲每个分量最大强度就是 256。
...
这里 $I_{max}$ 是光源最亮的颜色分量。我们使用光源最亮的颜色分量，因为求解光源最亮强度值的方程最能反映理想的光体积半径。
...
这给了我们一个一般方程，在已经常数项、一次线性参数项和二次参数项的情况下，允许我们计算光源的光体积半径 $d$：
...
我们计算场景中每个光源的这个半径，并仅当片段在光源的体积内时，才用它来计算光源的光照。下面是更新后的光照通道片段着色器，它把计算出的光体积考虑在内了。请注意，这种方法只用于教学目的，在实际环境中不可行，我们很快就会讨论：
...
上面的片段着色器在实践中并不真正起作用，它只说明了如何使用光源的体积来减少光照计算。现实情况是，你的 GPU 和 GLSL 在循环和分支上优化非常糟糕。原因是 GPU 上的着色器执行是高度并行的，而且绝大多数架构都要求大量线程必须运行完全相同的着色器代码才能提高效率。这通常意味着着色器运行时会执行 if 语句的所有分支，以确保该组线程的着色器运行相同，这使得我们之前的半径检查优化完全无用：我们仍会计算所有光源的光照！
...
使用光体积更好的方法是渲染按光体积半径缩放的实际球体。这些球体的中心在光源的位置上，当它按光体积半径缩放时，正好包含了光的可见体积。这就是技巧所在：我们使用延迟光照着色器来渲染球体。由于渲染球体区域像素与光源影响的像素完全一致，因此我们只渲染相关像素，跳过所有其他像素（现在是一个一个的在光源位置渲染体积球，而非之前渲染一整个填充屏幕的四边形）。下图说明了这一点：
...
渲染光源确实会对性能造成影响，虽然在渲染大量光源时，它通常比普通的延迟着色要快得多，但我们仍有很多可以优化的地方。在延迟着色的基础上，还有另外两种流行的（更高效的）扩展，即延迟光照和基于瓦片的延迟着色。这两种扩展在渲染大量光线时效率更高，而且还能实现相对高效的 MSAA。
...
当您的场景很小且光源不多时，延迟渲染不一定更快，有时甚至更慢，因为额外使用延迟渲染花费的开销会超过其带来的好处。在更复杂的场景中，延迟渲染很快成为了一项重要的优化;尤其是使用更高级的优化扩展。此外，某些渲染效果（尤其是后期处理效果）在延迟渲染管道上的成本会更低，因为很多场景输入已经可以从 g 缓冲区中获得。
